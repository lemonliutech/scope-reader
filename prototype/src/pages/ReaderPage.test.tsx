...
describe("ReaderPage", () => {
  it("shows placeholder when no publication is open", () => {
    render(<ReaderPage publication={null} chapter={null} locator={null} temporary={false} onOpenTarget={vi.fn()} onOpenLibrary={vi.fn()} onImport={vi.fn()} />);
    expect(screen.getByText("还没有打开图书")).toBeInTheDocument();
  });

  it("renders ChapterFrame when a chapter is provided", async () => {
    await act(async () => {
      render(<ReaderPage publication={inspection} chapter={chapter} locator={null} temporary={false} onOpenTarget={vi.fn()} onOpenLibrary={vi.fn()} onImport={vi.fn()} />);
    });
  ...
  it("shows a temporary notice when book cannot be saved", () => {
    render(<ReaderPage publication={inspection} chapter={chapter} locator={null} temporary={true} onOpenTarget={vi.fn()} onOpenLibrary={vi.fn()} onImport={vi.fn()} />);
    expect(screen.getByText(/本次进度无法保存/)).toBeInTheDocument();
  });

  it("shows reading order as navigation when no nav nodes exist", () => {
    render(<ReaderPage publication={inspection} chapter={null} locator={null} temporary={false} onOpenTarget={vi.fn()} onOpenLibrary={vi.fn()} onImport={vi.fn()} />);
